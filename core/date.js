...
;(function () {
	...
	if ((day == 0) || (day < month && day < year)) {
		notation = '%d.%m.%Y';
		patterns.push(yearMonthDay, monthDayYear, dayMonthYear);
	}
	else
		if (day < year) {
			notation = '%m/%d/%y';
			patterns.push(yearMonthDay, dayMonthYear, monthDayYear);
		}
		else {
			notation = '%Y-%m-%d';
			patterns.push(monthDayYear, dayMonthYear, yearMonthDay);
		}
	
	prototype.notation = notation;
	...
	// Detect military time
	prototype.military = !/\b(am|pm)\b/i.test(new Date().toLocaleTimeString());
})();


Date.prototype.parse = function(s) {
	var allDay = true,
		patterns = this.matching.patterns,
		methods = this.matching.methods;
